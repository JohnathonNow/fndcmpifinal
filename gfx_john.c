...
// Added by John for ease of use, Dec 10, 2015
static char current_font[80];
...
/* Disables key autorepeat in X11 */
void johngfx_disable_key_repeat()
{
	XkbSetDetectableAutoRepeat (gfx_display, 1, NULL);
}
...
/* Draws a line of text in a different font */
void johngfx_text_font( int x, int y , char *text , char *font_str )
{
   Font font;
   font = XLoadFont(gfx_display, font_str);
   XSetFont(gfx_display, gfx_gc, font);
   XDrawString(gfx_display, gfx_window, gfx_gc, x, y, text, strlen(text));
   font = XLoadFont(gfx_display, current_font);
   XSetFont(gfx_display, gfx_gc, font);
}
...
/* Gets the font used by gfx_text() */
char* johngfx_getfont()
{
   return current_font;
}
